...
class Sender extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5656445737175450250L;
    ...
    public Sender(JTabbedPane _notebook) {
        ...
        top.add(new JLabel(TCPMon.getMessage("connectionEndPoint00", "Connection Endpoint"), SwingConstants.RIGHT));
        top.add(Box.createRigidArea(new Dimension(5, 0)));
        ...
        top.add(new JLabel(TCPMon.getMessage("soapAction00", "SOAP Action  "), SwingConstants.RIGHT));
        ...
        bottomButtons.add(sendButton = new JButton(TCPMon.getMessage("send00", "Send")));
        bottomButtons.add(Box.createRigidArea(new Dimension(5, 0)));
        ...
        TCPMon.getMessage("close00", "Close");
        pane2.add(bottomButtons, BorderLayout.SOUTH);
        sendButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                if ("Enviar".equals(event.getActionCommand()) || "Send".equals(event.getActionCommand())) {
                    send();
                }
            }
        });
