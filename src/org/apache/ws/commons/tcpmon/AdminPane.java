...
class AdminPane extends JPanel {
	 ...
	 * Field sslServer
	 */
	public JCheckBox sslServer;

	/**
	 * Field sslLabel
	 */
	public JLabel sslLabel;
	...
	public AdminPane(JTabbedPane notebook, String name) {
		...
		c.gridwidth = 1;
		tmpPanel.add(
				new JLabel(TCPMon.getMessage("listenPort00", "Listen Port #")
						+ " "), c);
		c.anchor = GridBagConstraints.WEST;
		c.gridwidth = GridBagConstraints.REMAINDER;
		tmpPanel.add(port = new NumberField(4), c);
		port.setValue(TCPMon.DEFAULT_LOCAL_PORT);
		port.setToolTipText(TCPMon.getMessage("tiplistenport00",
				"Enter the local port for listenting"));
		port.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				Strings.setLocalPort(port.getText());
		...
		listenerButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (listener.equals(event.getActionCommand())) {
					...
					host.setToolTipText(TCPMon.getMessage(
							"tiptargetHostname00", "Enter the Target hostname"));
					tport.setToolTipText(TCPMon.getMessage("tiptargetPort00",
							"Enter the port of the target Host"));
					sslServer.setEnabled(state);
					sslLabel.setForeground(state ? Color.black : Color.gray);
				}
			}
		});
		c.anchor = GridBagConstraints.WEST;
		...
		host.setToolTipText(TCPMon.getMessage("tiptargetHostname00",
		...
		host.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				Strings.setRemoteHost(host.getText());
			}
		...
		tport.addKeyListener(new KeyListener() {
			...
			@Override
			public void keyReleased(KeyEvent e) {
				Strings.setRemotePort(tport.getText());
		...
		mainPane.add(Box.createRigidArea(new Dimension(25, 0)));
		mainPane.add(
				sslLabel = new JLabel(TCPMon.getMessage("sslSelected00",
					"SSL Server") + " "), c);
		c.anchor = GridBagConstraints.WEST;
		c.gridwidth = GridBagConstraints.REMAINDER;
		sslServer = new JCheckBox();
		mainPane.add(sslServer, c);
		sslServer.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				Strings.setSslServerSelected(sslServer.isSelected());
				if (Strings.isSslServerSelected()){
		...
		proxyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (proxy.equals(event.getActionCommand())) {
					...
					tport.setToolTipText(null);
					sslServer.setEnabled(!state);
					sslLabel.setForeground(state ? Color.gray : Color.black);
				}
			}
		});
		...
		HTTPProxyBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (proxySupport.equals(event.getActionCommand())) {
					...
					if (b) {
						HTTPProxyPort.setToolTipText(TCPMon.getMessage(
								"tipport00", "Enter the port of your proxy"));
						HTTPProxyHost.setToolTipText(TCPMon
								.getMessage("tiphostname00",
										"Enter the hostname or IP of the proxy that you are connected"));
					} else {
						HTTPProxyPort.setToolTipText(null);
						HTTPProxyHost.setToolTipText(null);
					}
		...
		mainPane.setBorder(new EmptyBorder(0, 50, 0, 0));
		...
		this.add(new JScrollPane(mainPane), BorderLayout.WEST);
